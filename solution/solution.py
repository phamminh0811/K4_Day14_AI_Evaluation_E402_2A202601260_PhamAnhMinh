...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            'object mapping criterion name to score, e.g. {"accuracy": 0.8}.'
        )
        ...
        try:
            ...
            if not isinstance(parsed, dict) or not parsed:
                raise ValueError("judge response is not a non-empty JSON object")
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        all_scores = [
            ...
            for score in entry.get("scores", {}).values()
        ]
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest = [name for name, score in scores.items() if score == min_score]
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                "Implement a hallucination checker / grounding guardrail to filter "
                "claims unsupported by retrieved context"
            )
        if categories.get("irrelevant"):
            suggestions.append(
                "Clarify prompt instructions and intent routing so answers directly "
                "address the question asked"
            )
        if categories.get("incomplete"):
            suggestions.append(
                "Increase chunk size or retrieval top-k, and add few-shot examples "
                "showing complete answers to reduce context fragmentation"
            )
        if categories.get("off_topic"):
            suggestions.append(
                "Improve intent/topic detection upstream of generation to keep "
                "answers on the requested subject"
            )
        if categories.get("refusal"):
            suggestions.append(
                "Loosen overly strict guardrails that cause unnecessary refusals "
                "on answerable in-scope questions"
            )

        fallback = [
            "Expand golden dataset coverage for the observed failure categories",
            "Review context precision to ensure the most relevant chunks rank first",
            "Add regression tests so these failure cases are re-checked on every release",
        ]
        for s in fallback:
